...
{
    // 计算当前互斥锁在进程锁池中的索引（id ∈ [0, LOCK_POOL_SIZE)）
    // 这是将物理锁对象映射为资源编号的关键步骤，便于在 allocation/request 矩阵中索引
    int id = m - curr_proc()->mutex_pool;
    struct proc *p = curr_proc();
    struct thread *t = curr_thread();

    // 尝试快速获取锁：若当前未被持有（locked == 0），则直接获取
    if (!m->locked) {
        m->locked = 1;  // 标记锁已被占用

        // 【关键：资源分配状态更新】
        // 银行家算法依赖准确的 "Allocation" 矩阵来模拟资源释放过程。
        // 此处将线程 t->tid 对资源 id 的占有状态设为 1，
        // 表示该线程当前持有一份互斥锁资源（每把锁视为单位资源）。
        // 注意：互斥锁是二值资源（总量=1），因此 allocation 值非0即1。
        p->mutex_allocation[t->tid][id] = 1;

        // 此时无需更新 request 矩阵，因为请求已满足，不再处于“等待”状态。
        // request 矩阵仅在 sys_mutex_lock 系统调用入口处临时设置，
        // 用于 deadlock_detect() 模拟“若此次请求被批准，系统是否仍安全”。
        return;
    }
}
```
...
死锁检测（尤其是基于银行家算法的动态检测）具有**运行时开销**：
- 每次尝试获取锁或信号量前，需遍历所有线程和资源池；
- 银行家算法的时间复杂度为 $O(n^2 m)$（$n$ 为线程数，$m$ 为资源类型数）；
- 在高并发或频繁同步的场景下，性能影响显著。

因此，**不能默认开启**死锁检测。  
`sys_enable_deadlock_detect(enable)` 的作用是：

> **允许用户程序按需启用/禁用死锁检测功能**，在调试阶段开启以捕获死锁，生产环境关闭以提升性能。

这体现了操作系统设计中 **“可配置性”与“性能/安全权衡”** 的经典思想。

#### 二、核心原理：如何检测死锁？

本实现采用 **银行家算法（Banker’s Algorithm）** 进行**预防性死锁检测**（注意：不是事后检测，而是在资源请求发生前预测是否会导致死锁）。

##### 银行家算法的核心假设：
- 系统知道每个线程**未来可能申请的最大资源量**（Max）；
- 但在 uCore Lab8 中，由于用户程序无法预声明 Max，我们退化为 **“当前请求 + 已持有 = 安全状态？”** 的简化模型。

##### 本实现的关键抽象：
将**互斥锁**和**信号量**统一视为**可计数资源**：
- **互斥锁**：每把锁是一个单位资源（总量 = 1），`locked == 1` 表示已被占用（Available = 0）；
- **信号量**：其 `count` 字段直接表示可用资源数量（Available = max(0, count)）。

于是，死锁检测问题转化为：
> **是否存在一个线程调度序列，使得所有线程都能完成并释放其持有的资源？**

若不存在，则说明当前资源请求会导致死锁。

#### 三、关键数据结构：如何跟踪资源状态？

为了支持银行家算法，每个进程（`struct proc`）维护以下四个 **$NTHREAD \times LOCK\_POOL\_SIZE$** 的整型矩阵：

| 矩阵                         | 含义                                         | 更新时机                                                     |
| ---------------------------- | -------------------------------------------- | ------------------------------------------------------------ |
| `mutex_allocation[tid][lid]` | 线程 `tid` 是否持有互斥锁 `lid`              | `mutex_lock` 成功时置 1；`mutex_unlock` 时清 0               |
| `mutex_request[tid][lid]`    | 线程 `tid` 是否正在等待互斥锁 `lid`          | 在 `sys_mutex_lock` 调用开始时临时置 1（用于模拟请求），检测完立即清除 |
| `sem_allocation[tid][sid]`   | 线程 `tid` 持有的信号量 `sid` 的“虚拟持有量” | 实际上信号量不绑定持有者，此处简化为：线程成功 `down` 后记为 1 |
| `sem_request[tid][sid]`      | 线程 `tid` 是否正在等待信号量 `sid`          | 在 `sys_semaphore_down` 开始时临时置 1，检测完清除           |

> **注意**：`request` 矩阵是**瞬时状态**，仅在系统调用入口处短暂设置，用于模拟“如果这次请求被满足，系统是否仍安全”。

同时，`deadlock_detect_enable` 标志位控制是否执行检测逻辑。

#### 四、算法集成时机：何时触发检测？

死锁检测**不在锁内部实现**，而是在**系统调用层**进行，原因如下：
- 系统调用是用户态到内核态的明确边界，便于统一拦截；
- 可在真正阻塞前进行预测，避免进入不可逆的等待状态。
...
int sys_mutex_lock(int mutex_id) {
    if (p->deadlock_detect_enable) {
        p->mutex_request[tid][mutex_id] = 1;     // 模拟请求
        if (deadlock_detect(p)) {                // 执行银行家算法
            p->mutex_request[tid][mutex_id] = 0;
            return -0xDEAD;                      // 返回特殊错误码
        }
        p->mutex_request[tid][mutex_id] = 0;     // 清除模拟请求
    }
    // ... 执行真正的 mutex_lock ...
}
```
同理应用于 `sys_semaphore_down`。

> **关键洞察**：这种“**请求前预测 + 安全放行**”的模式，是死锁**避免（Avoidance）** 策略，而非死锁**检测（Detection）**。但 Lab8 将其称为“死锁检测”，实为术语简化。

#### 五、实现细节与工程考量

1. **返回值 `-0xDEAD` 的意义**：
   - 十六进制 `0xDEAD` = 十进制 57005，取负值作为错误码；
   - 具有**自文档化**特性（"DEAD" 直观表示死锁），便于调试；
   - 用户程序可据此主动终止或回滚操作。

2. **资源可用性计算**：
   ```c
   work_mutex[i] = p->mutex_pool[i].locked ? 0 : 1;
   work_sem[i] = (p->semaphore_pool[i].count > 0) ? p->semaphore_pool[i].count : 0;
   ```
   - 互斥锁：未被持有（`locked == 0`）则 Available = 1；
   - 信号量：`count` 可能为负（表示等待者数量），此时 Available = 0。

3. **线程状态过滤**：
   ```c
   finish[i] = (thread unused or exited) ? 1 : 0;
   ```
   忽略已退出或未使用的线程，避免干扰安全序列判断。

4. **原子性保证**：
   - 整个检测过程在**内核态、关中断**（或至少持有大内核锁）下执行；
   - 确保 `allocation` / `request` 矩阵在检测期间不被其他 CPU 修改，保证快照一致性。

#### 总结

`sys_enable_deadlock_detect` 不仅仅是一个开关，它背后是一套完整的**运行时资源建模 + 安全状态预测**机制。其实现体现了：
- 对**死锁理论**（四条件、银行家算法）的准确理解；
- 对**同步原语语义**（锁/信号量的资源属性）的抽象能力；
- 对**系统性能与安全性权衡**的工程判断；
- 对**用户接口设计**（明确错误码、按需启用）的规范意识。
...
![image-20260103190955145](C:\Users\dihao\AppData\Roaming\Typora\typora-user-images\image-20260103190955145.png)
