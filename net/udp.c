...
static struct udp_pcb *
udp_pcb_select(ip_addr_t addr, uint16 port)
{
    struct udp_pcb *pcb;
...
static void
udp_input(const uint8 *data, uint len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (len != ntoh16(hdr->len)) { /* just to make sure */
        errorf("length error: len=%u, hdr->len=%u", len, ntoh16(hdr->len));
        return;
    }
    pseudo.src = src;
    pseudo.dst = dst;
    ...
    memmove(entry->data, hdr+1, entry->len);
...
int
udp_output(struct ip_endpoint *src, struct ip_endpoint *dst, const uint8 *data, uint len)
{
    uint8 *buf;
    struct udp_hdr *hdr;
    struct pseudo_hdr pseudo;
    ...
    if (ip_output(IP_PROTOCOL_UDP, (uint8 *)hdr, total, src->addr, dst->addr) == -1) {
...
int
udp_sendto(int id, uint8 *data, uint len, struct ip_endpoint *foreign)
{
    struct udp_pcb *pcb;
    struct ip_endpoint local;
...
udp_recvfrom(int id, uint8 *buf, uint size, struct ip_endpoint *foreign)
{
    ...
    while (1) {
        ...
        err = sched_sleep(&pcb->ctx, &mutex, 0);
        if (err) {
            debugf("interrupted");
            mutex_unlock(&mutex);
            net_errno = EINTR;
